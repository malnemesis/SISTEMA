...
Class Consultas
{
	...
	public function comprasultimos_30dias()
	{
		$sql="SELECT CONCAT(DAY(fecha_hora),'-',MONTH(fecha_hora)) as fecha,SUM(total_compra) as total FROM ingreso GROUP by fecha_hora ORDER BY fecha_hora DESC limit 0,30";
		return ejecutarConsulta($sql);
	}

	public function ventasultimos_30dias()
	{
		$sql="SELECT CONCAT(DAY(fecha_hora),'-',MONTH(fecha_hora)) as fecha,SUM(total_venta) as total FROM venta GROUP by fecha_hora ORDER BY fecha_hora DESC limit 0,30";
		return ejecutarConsulta($sql);
	}
}
